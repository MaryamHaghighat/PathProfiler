...
class TileWorker(Process):
    def __init__(self, queue_in, slide_path, tile_size, downsample, output, processing_fn, *processing_fn_args):
    ...
    def run(self):
        ...
        while True:
            ...
            x_y = data_in
            try:
                tile, _ = self.slide.read_region_ds(x_y, self.downsample, (self.tile_size, self.tile_size), normalize=False,
...
def process_tiles(slide_path, tile_size, downsample, mask_path, mask_magnification, mask_ratio, processing_fn,
    ...
    workers = [TileWorker(queue_in, slide_path, tile_size, downsample, output, processing_fn, *processing_fn_args) for _ in range(n_workers)]
